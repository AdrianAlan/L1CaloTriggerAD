...
def main(source_background,
    ...
    background = DataLoader(
        L1UCTRegionsDataset(source_background, device, **settings),
        batch_size=1,
        num_workers=0,
        shuffle=False)
    ...
    with trange(int(10*samples_per_cv), file=sys.stdout) as tr:
        ...
        for i, (X, y) in enumerate(background):
            ...
            y = y.to(device)
            distance = torch.norm(X, p=2).reshape(-1)
    ...
    for signal in source_signals:
        benchmark = DataLoader(
            L1UCTRegionsDataset(signal['path'], device, signal=True, **settings),
            batch_size=1,
            num_workers=0,
            shuffle=True
        )
        ...
        with trange(int(10*samples_per_cv), file=sys.stdout) as tr:
            ...
            for i, ((X, y), (Xb, _)) in enumerate(zip(benchmark, background)):
                if i >= limits:
                    fpr, tpr, _ = roc_curve(
                ...
                y = y.to(device)
                distance = torch.norm(X + Xb, p=2).reshape(-1)
    ...
    plt.xscale("log")
    plt.axvline(x=0.003, color="black", linestyle = '--', lw = 2)
    plt.legend(bbox_to_anchor=(1., 1.))
    plt.savefig('{0}/ROC-{1}'.format(plot_path, "Baseline"))
...
if __name__ == '__main__':
    ...
    main(config['dataset']['test_background'],
